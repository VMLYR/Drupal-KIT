# Rocketship theme: starter theme
...
starter_theme.theme. You don't need to touch these
...
The aliases used with these includes, are defined in starter_theme.theme. The
...
- `starter_theme.layouts.yml`: The regions are defined here for custom layouts
...
This can be done by attaching a library via the theme's `starter_theme.theme` file's `HOOK_preprocess_page` or in a Twig template, for example.
...
This is done by attaching a library via the theme's `starter_theme.theme` file's `HOOK_preprocess_page`
